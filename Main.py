...
class FacialEmotionMovieRecommender:
    def _init_(self):
        # Initialize the facial emotion recognizer
        self.emotion_detector = FER(mtcnn=True)

        # TMDB API credentials
        self.api_key = "******************" #Your API Key(TMDB)
        ...
        self.emotion_genre_map = {
            ...
            'fear': [35, 53],  # Comedy, Thriller
            'happy': [27, 16, 10402, 10749],  # Animation, Music, Romance, Fear
    ...
    def load_genre_data(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Failed to fetch genre data: {response.status_code}")
    ...
    def detect_emotion(self, image):
        ...
        try:
            # Convert the image to grayscale for better emotion detection
            ...
            for i, face_data in enumerate(emotions):
                ...
                # Get emotions probabilities
                emotions_prob = face_data['emotions']
    ...
    def fetch_movies_by_genre(self, genre_ids, page=1, min_rating=7.0):
        ...
        cache_key = f"genres_{'-'.join(map(str, genre_ids))}page{page}"
    ...
    def recommend_movies(self, emotion_results, num_recommendations=5):
        ...
        if not movies_data or "results" not in movies_data or len(movies_data["results"]) == 0:
            print(" No movies found for these genres. Trying with broader criteria...")
            movies_data = self.fetch_movies_by_genre(suitable_genre_ids, min_rating=6.0)

        if not movies_data or "results" not in movies_data or len(movies_data["results"]) == 0:
            print(" Still no movies found. Showing popular movies instead.")
